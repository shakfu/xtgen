 ...
 * such boundary a "bang" is emitted at the 2nd outlet and the counter value
 * wraps
 *
 ...
 * since the "set" message has one floating-point argument (as declared in
 * counter_setup()) we get a reference to the class-dataspace and the value
 */
...
void counter_setup(void) {
    counter_class = class_new(gensym("counter"), (t_newmethod)counter_new, 0,
                              sizeof(t_counter), CLASS_DEFAULT,
    ...
    /* call a function when a "reset" message (without arguments) appears on the
     * first inlet */
    class_addmethod(counter_class, (t_method)counter_reset, gensym("reset"), 0);

    /* call a function when a "set" message with one float-argument (defaults to
     * 0) appears on the first inlet */
    class_addmethod(counter_class, (t_method)counter_set, gensym("set"),
                    A_DEFFLOAT, 0);

    /* call a function when a "bound" message with 2 float-argument (both
     * default to 0) appears on the first inlet this is used for "list" messages
     * which appear on the 2nd inlet the magic is done in counter_new()
     */
    class_addmethod(counter_class, (t_method)counter_bound, gensym("bound"),
